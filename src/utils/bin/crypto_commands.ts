...
<div class="crypto-card p-2 mb-2 border-l-4 border-blue-500 pl-2">
  <div class="crypto-name font-bold">${result.name.charAt(0).toUpperCase() + result.name.slice(1)}</div>
  <div class="crypto-price">Price: $${formatPrice(result.price)}</div>
  <div class="crypto-change">24h: ${formatChange(result.change24h)}</div>
  <div class="crypto-market-cap">Market Cap: ${formatMarketCap(result.marketCap)}</div>
  <div class="crypto-updated text-xs text-gray-500">Updated: ${result.lastUpdated}</div>
</div>
`);
    } else {
